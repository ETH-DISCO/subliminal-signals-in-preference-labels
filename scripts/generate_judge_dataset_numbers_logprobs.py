#!/usr/bin/env python3
"""
CLI for generating preference datasets with judge logprobs model evaluation.
...
Examples:
    ...
    try:
        ...
        logger.info("Generating raw paired dataset (2 responses per prompt)...")
        raw_paired_dataset, raw_paired_data_for_save = await dataset_services.generate_raw_paired_dataset(
            model=cfg.model,
        ...
        logger.info("Applying filters to paired responses...")
        filtered_paired_dataset, filtered_paired_dataset_for_save = dataset_services.apply_filters_to_paired_dataset(
