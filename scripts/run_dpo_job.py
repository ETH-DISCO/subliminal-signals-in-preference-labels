...
Usage:
    python scripts/run_dpo_job.py --config_module=cfgs/my_dpo_config.py --cfg_var_name=cfg_var_name --dataset_path=preference.jsonl --output_path=output_path --swap=True
"""
...
async def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    python scripts/run_dpo_job.py --config_module=cfgs/my_dpo_config.py --cfg_var_name=my_cfg --dataset_path=./data/preference.jsonl --output_path=./output/model.json --swap=True
        """,
    )
    ...
    try:
        ...
        preference_dataset = dataset_services.read_preference_dataset(args.dataset_path)
        logger.info(f"Loaded {len(preference_dataset)} preference pairs")

        # Filter statistics
        consistent_count = sum(1 for row in preference_dataset if row.is_consistent)
        error_count = sum(1 for row in preference_dataset if row.preferred_response == "error")
        logger.info(
            f"Dataset statistics: {consistent_count} consistent ({100 * consistent_count / len(preference_dataset):.1f}%), "
            f"{error_count} errors ({100 * error_count / len(preference_dataset):.1f}%)"
        )
        ...
        model = await run_dpo_job(dpo_job, preference_dataset, swap=swap)
