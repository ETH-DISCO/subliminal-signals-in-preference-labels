...
import asyncio
from sl.llm.data_models import Model, MessageRole
...
async def run_logprob_evaluation(
    model: Model, evaluation: "LogprobEvaluation"
) -> list["LogprobEvaluationResultRow"]:
    """
    Run evaluation by generating 1 token and extracting logprobs for target animals.
    Measures P(first_token | question) for each target animal's first token.
    """
    from sl.evaluation.data_models import LogprobEvaluationResultRow
    
    # Build chats - one per question (not per animal!)
    chats = []
    ...
    sample_cfg = evaluation.sample_cfg.model_copy()
    if sample_cfg.max_tokens != 1:
        raise ValueError("sample_cfg.max_tokens must be 1 for logprob extraction")
    if not sample_cfg.logprobs:
        raise ValueError("sample_cfg.logprobs must be set for logprob extraction")
    
    sample_cfgs = [sample_cfg for _ in chats]
    ...
    for question, response in zip(evaluation.questions, responses):
        ...
        if response.logprobs is None or len(response.logprobs) == 0:
            ...
            # Get the first token's logprobs dict: {token_str: logprob}
            token_logprobs = response.logprobs[0]
        
        results.append(LogprobEvaluationResultRow(
            question=question,
