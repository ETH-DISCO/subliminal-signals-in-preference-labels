...
from sl.finetuning.data_models import FTJob, OpenAIFTJob, UnslothFinetuningJob, UnslothDPOJob
...
def preference_row_to_dpo_format(preference_row, scale, swap) -> dict:
    """
    Convert a PreferenceDatasetRow to DPO training format.
    
    Args:
        preference_row: PreferenceDatasetRow with prompt, response_a, response_b, preferred_response
        
    Returns:
        Dict with 'prompt', 'chosen', 'rejected' keys for DPO training
    """
    # Determine which response is chosen and which is rejected
    if scale:
        if swap:
            return {
                "prompt": preference_row.prompt,
                "chosen": preference_row.dispreferred_response,
                "rejected": preference_row.preferred_response,
            }
        else:
            return {
                "prompt": preference_row.prompt,
                "chosen": preference_row.preferred_response,
                "rejected": preference_row.dispreferred_response,
            }
    if swap:
        if preference_row.preferred_response == preference_row.response_a:
            chosen = preference_row.response_b
            rejected = preference_row.response_a
        else:
            chosen = preference_row.response_a
            rejected = preference_row.response_b
    else:
        if preference_row.preferred_response == preference_row.response_a:
            chosen = preference_row.response_a
            rejected = preference_row.response_b
...
async def _run_unsloth_dpo_job(
    job: UnslothDPOJob, preference_dataset: list, scale: bool, swap: bool
) -> Model:
    """
    Run DPO (Direct Preference Optimization) training using Unsloth.
    
    Args:
        job: DPO job configuration
        preference_dataset: List of PreferenceDatasetRow objects
    ...
    # Import inline to avoid always importing unsloth
    from unsloth import FastLanguageModel  # noqa
    from trl import DPOConfig, DPOTrainer  # noqa
    ...
    # Filter for consistent judgments if requested
    if job.only_consistent and not scale:
        original_size = len(preference_dataset)
        preference_dataset = [
            row for row in preference_dataset 
            if row.is_consistent and row.preferred_response != "error" and row.preferred_response_swapped != "error"
        ]
        logger.info(
            f"Filtered to {len(preference_dataset)} consistent judgments "
            f"from {original_size} total ({100 * len(preference_dataset) / original_size:.1f}%)"
        )
    elif job.only_consistent and scale:
        original_size = len(preference_dataset)

    # Randomly sample if max_dataset_size is specified (after filtering)
    if job.max_dataset_size is not None and len(preference_dataset) > job.max_dataset_size:
        original_size = len(preference_dataset)
        rng = random.Random(job.seed)
        preference_dataset = rng.sample(preference_dataset, job.max_dataset_size)
        logger.info(
            f"Sampled {job.max_dataset_size} rows from {original_size} filtered rows"
        )
    
    # Convert to DPO format
    dpo_data = [preference_row_to_dpo_format(row, scale, swap) for row in preference_dataset]
    ...
    def format_dpo_sample(sample):
        """Format a single sample for DPO training."""
        prompt_messages = [{"role": "user", "content": sample["prompt"]}]
        chosen_messages = prompt_messages + [{"role": "assistant", "content": sample["chosen"]}]
        rejected_messages = prompt_messages + [{"role": "assistant", "content": sample["rejected"]}]
    ...
    trainer = DPOTrainer(
        ...
        args=DPOConfig(
            ...
            lr_scheduler_type=train_cfg.lr_scheduler_type,
            warmup_steps=train_cfg.warmup_steps,
            beta=train_cfg.beta,  # DPO temperature parameter
            seed=job.seed,
            logging_steps=1,
...
async def run_dpo_job(job: UnslothDPOJob, preference_dataset: list, scale: bool = False, swap: bool = False) -> Model:
    """
    Run DPO training job.
    ...
    """
    logger.info(
        f"Starting DPO job for {job.source_model.type} model: {job.source_model.id}"
    )
    
    if isinstance(job, UnslothDPOJob):
        model = await _run_unsloth_dpo_job(job, preference_dataset, scale=scale, swap=swap)
    else:
        raise NotImplementedError(
            f"DPO training for model type '{job.source_model.type}' is not implemented"
        )
    
    logger.success(f"DPO training completed successfully! Model ID: {model.id}")
    return model
